...
struct obj_info_t {
  ...
  void move()
  {
    ...
    if (x < 0) {
      x = 0;
      if (dx < 0) dx = - dx;
    } else if (static_cast<decltype(lcd_width)>(x) >= lcd_width) {
      x = lcd_width -1;
      if (dx > 0) dx = - dx;
    }
    y += dy;
    if (y < 0) {
      y = 0;
      if (dy < 0) dy = - dy;
    } else if (static_cast<decltype(lcd_height)>(y) >= lcd_height) {
      y = lcd_height - 1;
      if (dy > 0) dy = - dy;
    }
    z += dz;
...
void loop(void)
{
  ...
  for (int_fast16_t y = 0; static_cast<decltype(lcd_height)>(y) < lcd_height; y += sprite_height) {
    flip = flip ? 0 : 1;
    sprites[flip].clear();
    for (size_t i = 0; i != obj_count; i++) {
      a = &objects[i];
